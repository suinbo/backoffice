...
import Section1 from "../uploadImage/section/Section1"
...
const CurationDetail = () => {
    ...
    useFetch<PageCodeList[]>(
        { url: `${API.SYSCODE_LIST}?uxId=${VIEW_CODES.MENU2}` },
        {
            onSuccess: (items: PageCodeList[]) => {
                ...
                setOpSelecBoxItems({
                    poc: imageTypes.map(item => ({ isChecked: true, label: item.name, value: item.value })),
    ...
    const naviItem = useMemo<Array<NavItem>>(
        () => [
            { link: "section1", title: t("section1") },
            { link: "section2", title: t("section2") },
            { link: "section3", title: t("section3") },
        ],
        [formItem]
    )
